class apb_device_env extends uvm_env;
  ...
  virtual apb_intf APB_VMINTF;
  virtual apb_intf APB_VSINTF;
  ...
  function void build_phase(uvm_phase phase);
    ...
    if (!uvm_config_db#(virtual apb_intf)::get(this, "", "APB_VMINTF", APB_VMINTF))
      `uvm_fatal("NOVIF", "virtual master interface connection failed!")
    uvm_config_db#(virtual apb_intf)::set(this, "apb_master_agent_1", "APB_VMINTF", APB_VMINTF);
    uvm_config_db#(virtual apb_intf)::set(this, "coverage_full", "APB_VMINTF", APB_VMINTF);

    if (!uvm_config_db#(virtual apb_intf)::get(this, "", "APB_VSINTF", APB_VSINTF))
      `uvm_fatal("NOVIF", "virtual slave interface connection failed!")
    uvm_config_db#(virtual apb_intf)::set(this, "apb_slave_agent_1", "APB_VSINTF", APB_VSINTF);
    uvm_config_db#(virtual apb_intf)::set(this, "coverage_full", "APB_VSINTF", APB_VSINTF);
  ...
  task reset_phase(uvm_phase phase);
    ...
    APB_VSINTF.presetn <= 1'b0;
    @(APB_VMINTF.master_cb);
    APB_VMINTF.presetn <= 1'b1;
    APB_VSINTF.presetn <= 1'b1;
    @(APB_VMINTF.master_cb);
