...
本节测试点在完成独立testcase及回归签核前统一视为`Planned`。当前基础`test`已经初步覆盖DIR-002、INIT-001、COPY-001、LEN-001、INT-001/003/004/006/011/012及部分SAPB测试点，但尚不能据此将对应测试点标记为closed。
...
|---|---|---|---:|---|
| `test` | direct、INIT、COPY、invalid LEN、invalid op、ISR smoke | Directed smoke | 1 | 已实现（基础） |
| `dma_apb_register_test` | REG、LEN、reserved区域 | Directed | 1 | Planned |
| `dma_apb_direct_test` | direct随机读写、back-to-back、地址/数据边界 | Directed + random | 10 | Planned |
| `dma_apb_init_test` | INIT长度、数据、地址和wait state | Directed + random | 10 | Planned |
| `dma_apb_copy_test` | COPY长度、数据、顺序和地址关系 | Directed + random | 20 | Planned |
| `dma_apb_interrupt_test` | 四类中断、组合状态和W1C | Directed | 5 | Planned |
| `dma_apb_busy_test` | busy期间各类CPU访问 | Directed | 10 | Planned |
| `dma_apb_reset_test` | APB及DMA各阶段复位 | Directed | 10 | Planned |
| `dma_apb_wait_state_test` | 上下游不同等待周期 | Constrained random | 20 | Planned |
| `dma_apb_random_test` | 混合direct/register/DMA流量 | Constrained random | ≥100 | Planned |
| `dma_apb_coverage_closure_test` | 定向补齐未命中bin | Directed | 按需 | Planned |
